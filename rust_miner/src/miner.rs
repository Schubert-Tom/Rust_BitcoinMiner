...
pub fn mine(extranonce1: &Vec<u8>, extranonce2_length: &u32, prevhash: &Vec<u8>, coinb1: &Vec<u8>, coinb2: &Vec<u8>, merkle_branches: &[Vec<u8>], version: &Vec<u8>, nbits: &u32, ntime: &Vec<u8>)->u32
{
    let nbits_vec = u32_u8(nbits);
    let coinbase = build_coinbase(&coinb1, &coinb2, &extranonce1, &extranonce2(extranonce2_length));
    ...
    while nonce < 0xFFFF //Limit
    {
        ...
        {
            println!("Jackpot, lucky number: {}", nonce);
            return nonce;
        }
        nonce = nonce + 1;
    }
...
pub fn test_miner()
{
    ...
    let clean_jobs = true;
    let n = mine(&ex1, &ex2_l, &revec(&phash), &coinb1, &coinb2, &m_branch_u8, &vers, &nbits, &ntime);
}